// OpenCode CLI 会话用量采集 CLI 入口：
//   npx tsx scripts/opencode-ingest.ts

import "dotenv/config";
import { prisma } from "@/lib/db";
import { ingestUsageFromOpencode } from "@/lib/opencode-ingest";

const LOCK_KEY = BigInt(491704); // 与 cliproxy/codex ingest 区分的 advisory lock key
...
async function main() {
  ...
  if (!locked) {
    console.warn("opencode:ingest skipped — another run is in progress (advisory lock held)");
    return;
  }
